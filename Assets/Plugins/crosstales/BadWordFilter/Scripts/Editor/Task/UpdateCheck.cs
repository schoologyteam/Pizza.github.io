...
namespace Crosstales.BWF.EditorTask
{
   ...
   public static class UpdateCheck
   {
      ...
      private static UpdateStatus _status = UpdateStatus.NOT_CHECKED;
      ...
      private static string[] readData()
      {
         ...
         try
         {
            ...
            using (System.Net.WebClient client = new Crosstales.Common.Util.CTWebClient())
            {
               ...
               {
                  data = line.Split(SPLIT_CHAR, System.StringSplitOptions.RemoveEmptyEntries);
      ...
      private static void updateStatus(string[] data)
      {
         if (data != null)
         {
            if (int.TryParse(data[1], out int buildNumber))
            {
               ...
               {
                  _status = UpdateStatus.UPDATE;
               }
               else
                  switch (buildNumber)
                  {
                     case -200:
                        _status = UpdateStatus.UPDATE_VERSION;
                        break;
                     case -900:
                        _status = UpdateStatus.DEPRECATED;
                        break;
                     default:
                        _status = UpdateStatus.NO_UPDATE;
                        break;
                  }
            }
