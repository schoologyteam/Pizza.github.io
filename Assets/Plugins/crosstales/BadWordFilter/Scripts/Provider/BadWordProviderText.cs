...
namespace Crosstales.BWF.Provider
{
   ...
   public class BadWordProviderText : BadWordProvider
   {
      ...
      private IEnumerator loadWeb(Crosstales.BWF.Data.Source src)
      {
         ...
         if (Crosstales.Common.Util.NetworkHelper.isURL(src.URL))
         {
            using (UnityWebRequest www = UnityWebRequest.Get(src.URL.Trim()))
            {
...
#endif
               {
                  ...
                  if (list.Count > 0)
                  {
                     _badwords.Add(new Crosstales.BWF.Model.BadWords(src, list));
                     _webSuccess = true;
                  }
                  else
                  {
         ...
         {
            Debug.LogWarning($"'URL' is invalid, null or empty!{System.Environment.NewLine}Please add a valid URL.", this);
         }
