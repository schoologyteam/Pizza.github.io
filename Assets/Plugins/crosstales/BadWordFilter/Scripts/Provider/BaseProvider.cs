...
namespace Crosstales.BWF.Provider
{
   ...
   public abstract class BaseProvider : MonoBehaviour, IProvider
   {
      ...
      public System.Collections.Generic.List<string> Verify(Crosstales.BWF.Data.Source source)
      {
         System.Collections.Generic.List<string> errors = new System.Collections.Generic.List<string>();

         if (source != null && source.RegexCount > 0)
         {
            foreach (string line in source.Regexes)
            {
               try
               {
                  System.Text.RegularExpressions.Regex tempRegex = new System.Text.RegularExpressions.Regex(line, RegexOption1 | RegexOption2 | RegexOption3 | RegexOption4 | RegexOption5);
               }
               catch (System.Exception ex)
               {
                  errors.Add($"Error in regex '{line}' from source '{source.SourceName}': {ex}");
               }
            }
         }
         else
         {
            errors.Add("Source contains no regexes!");
         }
